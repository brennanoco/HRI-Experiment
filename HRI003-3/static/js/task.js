...
myCondition = Math.floor(Math.random() * 4);
...
var speech_vid = prefix + "/speech/" + video_conditions[myCondition][0]; // This builds the path of the video we want to show currently
var motion_vid = prefix + "/motion/" + video_conditions[myCondition][1]; // This builds the path of the video we want to show currently
var question_label = video_conditions[myCondition][0] + video_conditions[myCondition][1]; // This is just the label of the video so the database is more readable
...
var DemoQuestionnaire = function() {
	...
	currentview = new VidCheck(); //should be VidCheck
    });
};
...
var CommandVideo = function() {
    ...
    record_responses = function() {
        psiTurk.recordTrialData({'phase':'speech_video', 'status':'submit'});
    };
    ...
    psiTurk.showPage('speech_video.html');
    window.scrollTo(0, 0);
    psiTurk.recordTrialData({'phase':'speech_video', 'status':'begin'});


    $("#mp4src").attr("src", speech_vid+".mp4")
    //$("#oggsrc").attr("src", speech_vid+".ogg")

    $("#video2").load();


    $("#video2").on('ended', function() {
        psiTurk.recordTrialData({'phase':'speech_video', 'status':'video ended'});
        $('#next').removeAttr('disabled');
    });

    $("#ppbutton").click(function () {
        psiTurk.recordTrialData({'phase':'speech_video', 'status':'play/pause clicked: '+ppcounter});
        ppcounter += 1;
    });

    $("#rsbutton").click(function () {
        psiTurk.recordTrialData({'phase':'speech_video', 'status':'restart clicked: '+rscounter});
        rscounter += 1;
    });
...
var ResponseVideo = function() {
    ...
    record_responses = function() {
        psiTurk.recordTrialData({'phase':'motion_video', 'status':'submit'});
    };
    ...
    // Load the questionnaire snippet
    psiTurk.showPage('motion_video.html');
    window.scrollTo(0, 0);
    psiTurk.recordTrialData({'phase':'motion_video', 'status':'begin'});


    $("#mp4src").attr("src", motion_vid+".mp4")
    //$("#oggsrc").attr("src", motion_vid+".ogg")

    $("#video3").load();


    $("#video3").on('ended', function() {
        psiTurk.recordTrialData({'phase':'motion_video', 'status':'video ended'});
        $('#next').removeAttr('disabled');
    });

    $("#ppbutton").click(function () {
        psiTurk.recordTrialData({'phase':'motion_video', 'status':'play/pause clicked: '+ppcounter});
        ppcounter += 1;
    });

    $("#rsbutton").click(function () {
        psiTurk.recordTrialData({'phase':'motion_video', 'status':'restart clicked: '+rscounter});
        rscounter += 1;
    });
